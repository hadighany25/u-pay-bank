...
// Function សម្រាប់ Format ថ្ងៃខែ (ចំណុចទី ៥)
...
const generateOfficialReceiptPDF = async (transactionId) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const tmpDir = path.join(__dirname, "../public/tmp"); // សម្រាប់ទុក QR Code មួយភ្លែត
      ...
      // -- 4. TRANSACTION DETAILS (ចំណុចទី ១: លុប continued: true ចេញ) --
      ...
      // Function គូរ Row (ចំណុចទី ២: គណនា Math.max(labelHeight, valueHeight))
      ...
      // មុខងារថ្មី ៣ ទៀត (Payment Method, Merchant ID, Invoice)
      ...
      // -- 5. Digital Signature (មុខងារថ្មីទី ៥) --
      ...
      // -- 6. QR Code (ចំណុចទី ៣: ប្រើ toFile ជំនួស DataURL) --
      const verifyLink = `https://u-pay-bank.fly.dev/receipt/${transaction.referenceId}`;
      const tmpQrPath = path.join(tmpDir, `qr-${transaction.referenceId}.png`);

      await QRCode.toFile(tmpQrPath, verifyLink, { margin: 1, width: 150 });
      doc.image(tmpQrPath, 485, currentY - 15, { width: 70 });
      doc
        .font("En-Medium")
        .fontSize(7)
        .fillColor("#718096")
      ...
      // -- 7. គូរត្រា VERIFIED --
      const stampX = 350; // រំកិលត្រាមកកណ្តាលបន្តិចកុំអោយបុក Signature
      ...
      // -- 8. Footer (ចំណុចទី ៤: ទប់កុំអោយ Email វែងធ្លាយ) --
      ...
      stream.on("finish", () => {
        // លុប File QR ព្រោះលែងត្រូវការវាទៀតហើយ
        if (fs.existsSync(tmpQrPath)) fs.unlinkSync(tmpQrPath);

        const publicUrl = `https://u-pay-bank.fly.dev/receipts/${fileName}`;
        resolve(publicUrl);
      });
    } catch (error) {
      reject(error);

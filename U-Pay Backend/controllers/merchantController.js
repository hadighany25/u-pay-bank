...
exports.createMerchant = async (req, res) => {
  try {
    ...
    console.log("Merchant saved successfully:", savedMerchant); // ដាក់ log នេះដើម្បីឆែកមើល

    res.status(201).json({
      success: true,

      merchant: {
        id: savedMerchant._id, // ត្រូវប្រាកដថាប្រើ _id (នេះជាអ្វីដែលកូដ Front-end រង់ចាំ)
...
exports.getMyMerchants = async (req, res) => {
  try {
    const userId = req.user.id || req.user._id; // ការពារកុំឱ្យវា undefined

  ...
  if (
    ...
    if (timeMatch) {
      ...
      if (
        ...
        return {
          allowed: false,
          message: `បម្រាម៖ អ្នកនៅក្រៅម៉ោងធ្វើការ! (ម៉ោងអនុញ្ញាតគឺ ${adminAcc.permissions.workStart} ដល់ ${adminAcc.permissions.workEnd}) 🛑`,
        };
      }
    }
  }
...
// ========================================================
// អនុវត្តមុខងារធម្មតា ដោយភ្ជាប់ប្រព័ន្ធ Log
// ========================================================
...
const getAdminLogs = async (req, res) => {
  try {
    // ត្រួតពិនិត្យថាមានតែ Super Admin ទេ ទើបអាចមើលបាន
    if (req.admin.role !== "super_admin")
      return res.status(403).json({ success: false, message: "Forbidden" });

    // ទាញយកប្រវត្តិ ១០០ ដងចុងក្រោយ បង្ហាញថ្មីបំផុតមុន
